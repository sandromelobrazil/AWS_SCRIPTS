...
sed -e "s/SEDsshuserSED/$sshuser/g" -e "s/SEDssherpasswordSED/$ssherpassword/g" -e "s/SEDrootpasswordSED/$rootpassword/g" -e "s/SEDemailaddressSED/$emailaddress/g" install_template.sh > install.sh
...
echo
echo then ssh to the box with:
echo ssh -p "$sshport" "$sshuser"@"$ip_address"
echo "enter password ($ssherpassword) and an MFA code to sign in"
echo "'sudo su' needs the root password ($rootpassword)"
echo
