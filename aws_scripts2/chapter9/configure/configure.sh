...
# this script configures the 2 internal servers

# it will zip up all the configure/internal* folders
...
# this script needs to be called from chapter9/configure directory
# with ./configure.sh
...
aws ec2 authorize-security-group-ingress --group-id $internal1sgid --source-group $bastionsgid --protocol tcp --port 22
...
aws ec2 authorize-security-group-ingress --group-id $internal2sgid --source-group $bastionsgid --protocol tcp --port 22
...
# allow access on 514 (rsyslog) from internal1 to bastion
aws ec2 authorize-security-group-ingress --group-id $bastionsgid --source-group $internal1sgid --protocol tcp --port 514

# allow access on 514 (rsyslog) from internal2 to bastion
aws ec2 authorize-security-group-ingress --group-id $bastionsgid --source-group $internal2sgid --protocol tcp --port 514

# make a vars.sh file from globals.sh and chapter9/vars.sh
# we are in chapter9/configure directory
...
# note agent forwarding does not work if you're in 'sudo su'
echo "#!/usr/bin/expect -f" > expect.sh
echo "set timeout -1" >> expect.sh
...
echo "expect \"finished install.sh on bastion\"" >> expect.sh
echo "send \"cd ..\n\"" >> expect.sh
echo "expect \"]\"" >> expect.sh
echo "send \"rm -f -r *\n\"" >> expect.sh
echo "expect \"]\"" >> expect.sh
...
# delete internal2.pem (MFA now installed)
cd ../internal/credentials
rm -f internal2.pem
cd ../../configure
...
aws ec2 revoke-security-group-ingress --group-id $internal1sgid --source-group $bastionsgid --protocol tcp --port 22

# revoke ssh between bastion and internal2
aws ec2 revoke-security-group-ingress --group-id $internal2sgid --source-group $bastionsgid --protocol tcp --port 22
