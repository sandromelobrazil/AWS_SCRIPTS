...
if test "$where" = "aws"; then
 ...
 echo "must be run from aws directory with ./ami/webphp/make.sh <N>"
 exit
fi
...
echo "building web$webid"

echo "check web$webid does not exist"
exists=$(aws ec2 describe-key-pairs --key-names web$webid --output text --query 'KeyPairs[*].KeyName' 2>/dev/null)
